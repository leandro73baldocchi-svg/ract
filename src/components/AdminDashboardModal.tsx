          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-base flex items-center gap-2">
                Painel de Controle Editorial Completo
            ...
            <div className="flex items-center gap-2 px-5 pt-2.5 border-b border-stone-200 dark:border-stone-800 bg-stone-50/70 dark:bg-stone-900/40 shrink-0 overflow-x-auto">
              <button
                ...
                className={`pb-2.5 px-3 text-xs font-semibold flex items-center gap-1.5 border-b-2 transition-colors cursor-pointer shrink-0 ${
                  activeTab === 'articles' ? 'border-stone-900 dark:border-stone-100 text-stone-900 dark:text-stone-100' : 'border-transparent text-stone-500 hover:text-stone-800 dark:hover:text-stone-300'
                }`}
              >
              ...
              <button
                ...
                className={`pb-2.5 px-3 text-xs font-semibold flex items-center gap-1.5 border-b-2 transition-colors cursor-pointer shrink-0 ${
                  activeTab === 'categories' ? 'border-stone-900 dark:border-stone-100 text-stone-900 dark:text-stone-100' : 'border-transparent text-stone-500 hover:text-stone-800 dark:hover:text-stone-300'
                }`}
              >
              ...
              <button
                ...
                className={`pb-2.5 px-3 text-xs font-semibold flex items-center gap-1.5 border-b-2 transition-colors cursor-pointer shrink-0 ${
                  activeTab === 'backup' ? 'border-stone-900 dark:border-stone-100 text-stone-900 dark:text-stone-100' : 'border-transparent text-stone-500 hover:text-stone-800 dark:hover:text-stone-300'
                }`}
              >
            ...
            <div id="admin-scrollable-content" className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                  ...
                  <form onSubmit={handleSaveArticle} className="p-4 sm:p-5 bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800 rounded-xl space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <div className="sm:col-span-2">
                        ...
                        <input type="text" required value={articleForm.titlePt} onChange={(e) => setArticleForm({ ...articleForm, titlePt: e.target.value })} placeholder="Ex: Novo telescópio espacial mapeia atmosfera de exoplanetas" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded focus:outline-none focus:border-stone-900 dark:focus:border-stone-300" />
                      </div>
                      ...
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Periódico / Fonte / Agência de Notícias</label>
                        <input type="text" value={articleForm.source} onChange={(e) => setArticleForm({ ...articleForm, source: e.target.value })} placeholder="Ex: Nature, Reuters, Agência FAPESP, MIT..." className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>

                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Autor(es) / Jornalista / Pesquisador</label>
                        <input type="text" value={articleForm.author} onChange={(e) => setArticleForm({ ...articleForm, author: e.target.value })} placeholder="Ex: Dra. Alice Smith ou Redação" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>

                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Link Original da Notícia / DOI / PDF</label>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <textarea required rows={3} value={articleForm.summaryPt} onChange={(e) => setArticleForm({ ...articleForm, summaryPt: e.target.value })} placeholder="Síntese da notícia, descobertas e impacto científico..." className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded resize-y" />
                      </div>

                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Ponto Chave / Conclusão Prática</label>
                        <input type="text" value={articleForm.keyTakeaway} onChange={(e) => setArticleForm({ ...articleForm, keyTakeaway: e.target.value })} placeholder="Ex: Primeira evidência direta de água na estratosfera." className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>

                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Tags (separadas por vírgula)</label>
                        <input type="text" value={articleForm.tags} onChange={(e) => setArticleForm({ ...articleForm, tags: e.target.value })} placeholder="Astronomia, NASA, Exoplanetas" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>
                    </div>
                        ...
                        displayedArticles.map((art) => (
                          <div key={art.id} className={`p-3 sm:p-3.5 flex items-start justify-between gap-3 hover:bg-stone-50 dark:hover:bg-stone-900/40 transition-colors ${isEditingId === art.id ? 'bg-amber-50/50 dark:bg-amber-950/20' : ''}`}>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[11px] text-stone-600 dark:text-stone-400 line-clamp-2 mt-0.5">{art.summaryPt || art.summary}</p>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button onClick={() => handleStartEdit(art)} className="p-1.5 rounded text-stone-600 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-800 transition-colors cursor-pointer" title="Editar artigo">
                                <Edit className="w-3.5 h-3.5" />
                              </button>
                              {art.link && (
                                <a href={art.link} target="_blank" rel="noreferrer" className="p-1.5 rounded text-stone-400 hover:text-stone-700 dark:hover:text-stone-200 transition-colors" title="Abrir link original">
                                  <ExternalLink className="w-3.5 h-3.5" />
                                </a>
                              )}
                              <button onClick={() => handleDeleteArticle(art.id, art.titlePt || art.title)} className="p-1.5 rounded text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors cursor-pointer" title="Excluir artigo do portal">
              ...
              {activeTab === 'feeds' && (
                <div className="space-y-6">
                  {feedSuccessMsg && (
                    <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-lg text-xs flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 shrink-0" /> <span>{feedSuccessMsg}</span>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleAddRssFeed} className="p-4 bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800 rounded-xl space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-stone-800 dark:text-stone-200 flex items-center gap-1.5">
                      <Plus className="w-3.5 h-3.5 text-blue-600" /> Adicionar Nova Fonte RSS / Agência
                    </h4>

                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Nome da Agência / Revista *</label>
                        <input type="text" required value={feedName} onChange={(e) => setFeedName(e.target.value)} placeholder="Ex: Agência FAPESP" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">URL do Feed RSS (XML) *</label>
                        <input type="url" required value={feedUrl} onChange={(e) => setFeedUrl(e.target.value)} placeholder="https://exemplo.com/feed.xml" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded font-mono" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Área / Categoria</label>
                        <select value={feedCategory} onChange={(e) => setFeedCategory(e.target.value)} className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded">
                          {allAvailableCategories.map((c) => (<option key={c.id} value={c.id}>{c.label}</option>))}
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 flex justify-end">
                      <button type="submit" className="px-4 py-2 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-lg text-xs font-semibold hover:bg-stone-800 dark:hover:bg-white transition-colors cursor-pointer flex items-center gap-1.5">
                        <Save className="w-3.5 h-3.5" /> Salvar Fonte de Notícias
                      </button>
                    </div>
                  </form>
                    ...
                    <div className="border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden bg-white dark:bg-stone-950 divide-y divide-stone-200 dark:divide-stone-800">
                      {rssFeeds.map((feed) => (
                        <div key={feed.id} className="p-3 flex items-center justify-between gap-3 text-xs">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-stone-900 dark:text-stone-100">{feed.name}</span>
                              <span className="px-1.5 py-0.5 rounded text-[10px] uppercase font-mono bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300">{feed.category}</span>
                            </div>
              ...
              {activeTab === 'categories' && (
                <div className="space-y-6">
                  ...
                  <form onSubmit={handleAddCategory} className={`p-4 sm:p-5 border rounded-xl space-y-3 ${isEditingCategory ? 'bg-amber-50/50 border-amber-200' : 'bg-stone-50 border-stone-200 dark:bg-stone-900/60 dark:border-stone-800'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        ...
                        <input type="text" required value={newCatLabel} onChange={(e) => setNewCatLabel(e.target.value)} placeholder="Ex: Robótica" className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded" />
                      </div>
                      ...
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1 text-blue-700 dark:text-blue-400">Posição no Menu (1, 2, 3...)</label>
                        <input type="number" min="1" value={newCatOrder} onChange={(e) => setNewCatOrder(e.target.value)} className="w-full px-3 py-1.5 text-xs bg-white dark:bg-stone-950 border border-blue-300 dark:border-blue-700 rounded font-bold text-center text-blue-700 dark:text-blue-400" />
                      </div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-stone-800 dark:text-stone-200">
                      Áreas Ativas (Listadas pela ordem de exibição)
                    </h4>
              ...
              {activeTab === 'backup' && (
                <div className="space-y-6">
                  <div className="p-4 sm:p-5 bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800 rounded-xl space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-stone-800 dark:text-stone-200 flex items-center gap-1.5">
                      <Database className="w-3.5 h-3.5 text-emerald-600" /> Backup Completo
                    </h4>
                    <p className="text-xs text-stone-500 dark:text-stone-400">
                      Baixe um arquivo com todos os artigos ({articlesList.length}), áreas e feeds cadastrados.
                    </p>

                    <div className="flex items-center gap-3 pt-2 flex-wrap">
                      <button onClick={handleExportBackup} className="px-3.5 py-2 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-lg text-xs font-semibold flex items-center gap-1.5 cursor-pointer hover:bg-stone-800 dark:hover:bg-white">
                      ...
                      <label className="px-3.5 py-2 bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-700 rounded-lg text-xs font-semibold flex items-center gap-1.5 cursor-pointer hover:bg-stone-100">
                        <Upload className="w-3.5 h-3.5" /> Restaurar Arquivo
                  ...
                  <form onSubmit={handleChangePassword} className="p-4 sm:p-5 bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800 rounded-xl space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-stone-800 dark:text-stone-200 flex items-center gap-1.5">
                      <Lock className="w-3.5 h-3.5 text-amber-600" /> Alterar Senha
                    </h4>
                    ...
                    <div className="max-w-xs space-y-2">
                      ...
                      <button type="submit" className="px-3.5 py-1.5 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded text-xs font-semibold cursor-pointer">
                        Salvar Nova Senha
                      </button>
                    </div>
                  </form>
