              ...
              {activeTab === 'affiliates' && (
                <div className="space-y-6">
                  ...
                  <form onSubmit={handleSaveAffiliate} className="p-4 sm:p-5 bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800 rounded-xl space-y-4">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 flex items-center gap-1.5"><TrendingUp className="w-4 h-4" /> Configurar Link de Afiliado Amazon</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="sm:col-span-2">
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Área do Artigo *</label>
                        <select value={affCatId} onChange={(e) => setAffCatId(e.target.value)} className="w-full px-3 py-2 text-xs bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 rounded focus:outline-none">
                          <option value="default">📘 Padrão Geral (Exibe se a área não tiver um link)</option>
                      ...
                      <div>
                        <label className="block text-[11px] font-semibold text-stone-700 dark:text-stone-300 mb-1">Seu Link (amzn.to) *</label>
                    ...
                    <div className="pt-2 flex justify-end"><button type="submit" className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs font-semibold transition-colors cursor-pointer flex items-center gap-1.5"><Save className="w-3.5 h-3.5" /> Salvar Recomendação</button></div>
                  </form>
                  
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-stone-800 dark:text-stone-200">Links Ativos ({affiliatesList.length})</h4>
                    <div className="border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden bg-white dark:bg-stone-950 divide-y divide-stone-200 dark:divide-stone-800">
                      {affiliatesList.length === 0 ? (
                        ...
                        affiliatesList.map((aff) => {
                          ...
                          return (
                            <div key={aff.id} className="p-3 flex items-center justify-between gap-3 text-xs"><div className="min-w-0"><div className="flex items-center gap-2 mb-0.5"><span className={`px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${aff.id === 'default' ? 'bg-amber-100 text-amber-800' : 'bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300'}`}>{categoryName}</span></div><p className="font-bold text-stone-900 dark:text-stone-100 truncate">{aff.title}</p><a href={aff.url} target="_blank" rel="noreferrer" className="text-[10px] text-blue-600 dark:text-blue-400 font-mono truncate hover:underline">{aff.url}</a></div><button onClick={() => handleStartEditAffiliate(aff)} className="p-1.5 rounded text-stone-600 hover:bg-stone-200 dark:text-stone-400 dark:hover:bg-stone-800 transition-colors"><Edit className="w-4 h-4" /></button></div>
                          );
                        })
                      )}
                    </div>
